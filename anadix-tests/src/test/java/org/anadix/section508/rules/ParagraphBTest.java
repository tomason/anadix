...
public class ParagraphBTest extends RulesetTest {
	...
	public void testRule3_1() {
		ParamTag tag = factory.createParamTag(dummyId, body, dummyAttributes);
		tag.setSource(dummySource);
		Collection<ReportItem> items = evaluate(tag);
	...
	public void testRule4_1() {
		HtmlElement tag = factory.createTag(dummyId, "embed", body, dummyAttributes);
		tag.setSource(dummySource);
		Collection<ReportItem> items = evaluate(tag);
	...
	public void testRule4_2() {
		HtmlElement tag = factory.createTag(dummyId, "table", body, dummyAttributes);
		tag.setSource(dummySource);
		Collection<ReportItem> items = evaluate(tag);
